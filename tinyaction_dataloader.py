...
class TinyVirat(Dataset):
    def __init__(self, cfg, data_split, data_percentage, num_frames, skip_frames, input_size, shuffle=False):
        ...
        self.class_labels = [k for k, v in
                             sorted(json.load(open(cfg.class_map, 'r')).items(), key=lambda item: item[1])]
    ...
    def load_frames_random(self, video_path):
        ...
        frame_indicies = [indx for indx in
                          range(random_start, random_start + self.num_frames * skip_frames, skip_frames)]
        ret = True
        counter = 0
        frames = []
        while ret:
